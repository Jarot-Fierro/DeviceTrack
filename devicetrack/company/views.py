...
def save_history(request, instance, old_instance, action):
    ...
    data = {
        'id_company': str(instance.id_company),
        'operation': action,
...
class BaseCompanyFormView(TemplateView, FormMixin):
...
class CompanyCreateView(BaseCompanyFormView, ProcessFormView):
...
class CompanyUpdateView(BaseCompanyFormView, ProcessFormView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['id_company'] = self.get_object().id_company
        return context
...
class CompanyToggleStatusView(View):
...
class CompanyHistoryView(ListView):
    ...
    template_name = 'company_list_history.html'

    def get_queryset(self):
        return CompanyHistory.objects.all().order_by('-updated_at')
